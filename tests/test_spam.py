...
class TestHasSuspiciousContent:
    ...
    @staticmethod
    def test_tinyurl_is_suspicious():
        assert _has_suspicious_content("Visit https://tinyurl.com/xyz") is True

    @staticmethod
    def test_normal_github_url_is_not_suspicious():
...
class TestCalculateSpamScore:
    @staticmethod
    def test_clean_user_has_zero_score():
        user = make_user()
    ...
    @staticmethod
    def test_very_few_followers_adds_score():
    ...
    @staticmethod
    def test_account_29_days_old_is_new():
        user = make_user(days_old=29)
        _, reasons = calculate_spam_score(user)
        assert any("days old" in r for r in reasons)

    @staticmethod
    def test_account_30_days_old_is_not_new():
        user = make_user(days_old=30)
        _, reasons = calculate_spam_score(user)
        assert not any("days old" in r for r in reasons)

    @staticmethod
    def test_suspicious_bio_adds_score():
    ...
    @staticmethod
    def test_suspicious_name_adds_score():
    ...
    @staticmethod
    def test_suspicious_in_both_name_and_bio_scored_once():
        user = make_user(name="aB3kQz9mNpXrTy", bio="crypto airdrop")
        _, reasons_both = calculate_spam_score(user)
        # suspicious_profile is counted only once even when both name and bio match
        suspicious_score_both = sum(2 for r in reasons_both if "suspicious content" in r)
...
class TestIsSpam:
    @staticmethod
    def test_clean_user_is_not_spam():
        user = make_user()
    ...
    @staticmethod
    def test_obvious_spam_is_detected():
        user = make_user(name=None, bio=None, public_repos=0, followers=0, days_old=5)
        spam, reasons = is_spam(user)
        assert spam is True
    ...
    @staticmethod
    def test_spam_with_suspicious_bio():
    ...
    @staticmethod
    def test_borderline_user_single_signal_not_spam():
    ...
    @staticmethod
    def test_returns_reasons_list_when_spam():
    ...
    @staticmethod
    def test_returns_empty_reasons_when_clean():
        user = make_user()
        spam, reasons = is_spam(user)
        assert spam is False
        assert reasons == []
